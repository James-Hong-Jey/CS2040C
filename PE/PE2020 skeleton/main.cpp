...
{
    // Task 1
    if (_tail == NULL) // same as _head == NULL
    {
        insertHead(n); return;
    }
...
int qBalancedBrackets(string exp) {
    ...
    */
    ...
    return brackets.empty() && braces.empty();
};

void Graph::DFS(int s, List<int>& output, bool resetVisited)
{
    // Task 5
    if (resetVisited) { _resetVisited(); }
    // Process the current node
    int node = s;
    _setVisited(node);
    output.insertTail(node);

    for (_al[node].start(); !_al[node].end(); _al[node].next()) {
        // each neighbour
        if (!_isVisited(_al[node].current()))
            DFS(_al[node].current(), output, false);
    }
    return;
};
...
void Graph::BFS(int s, List<int>& output, bool resetVisited)
{
    ...
    while (!order.empty()) {
        ...
        for (_al[node].start(); !_al[node].end(); _al[node].next()) {
            // each neighbour
            if (!_visited[_al[node].current()])
                order.enqueue(_al[node].current());
        }
    }
};
...
int Graph::nComponents() {
    // Task 7
    int components = 0;
    List<int> output; // serves no purpose btw
    // If _nVisited < _nv, search for unvisited nodes and start a DFS
    while (_nVisited < _nv) {
        components++;
        // find an unvisited node
        int node = 0;
        for (node; _isVisited(node); node++);

        // Run a DFS
        DFS(node, output, false);
    }
    return components;
};
///////////////////////////////////////////////////////////////////
